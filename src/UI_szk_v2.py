...
import sys_msg
from main_chat_agent_v2 import ChatAgent

# dotenv加载环境变量
load_dotenv(".env")
print("openai-api-key:", os.environ["GLM_API_KEY"])
...
with gradio.Blocks() as demo:
    gradio.HTML("<h1 align='center'>个性化共情对话系统</h1> ")
    gradio.HTML("<h4 align='center'>1.使用前请先输入您的用户名账号,并点击新建/读取对话按钮，第一次使用本系统时将新建对话，若使用过本系统则读取用户历史对话数据。2.使用时请在下方文本框中输入问题，点击发送提交消息，如需修改您上一条发送的消息可点击重新编辑以修改，点击清除历史以清除所有的对话历史，底部菜单可下拉以更换对话模型和调节模型生成回复的参数。3.使用完毕可点击保存当前对话按钮保存目前的对话历史，祝您心情愉快^_^</h4>")
    chat_history = []
    with gradio.Row():
        usr_msg = gradio.Textbox(label="请输入账号+用户名，作为管理对话历史的用户凭证（请使用数字开头，如2020123456李华）")
        with gradio.Column():
    ...
    with gradio.Column():
        ...
        with gradio.Accordion("对话模型选择及生成参数修改", open=False):
            models_choise = gradio.Dropdown(choices=["ESconv-smile", "psy-insight-Chinese", "psy-insight-English", "GLM4"], label="对话模型选择", value="ESconv-smile", allow_custom_value=True)
    ...
    def model_select(models):
        if models == 'ESconv-smile':
            chat_agent.chat_model = ChatOpenAI(
                model="glm-4",
                openai_api_key=os.environ["GLM_API_KEY"],
                openai_api_base=GLM_BASE_URL
            )
